...
int APIENTRY WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	memset(&piProcessB, 0, sizeof(piProcessB));
	memset(&piProcessC, 0, sizeof(piProcessC));
	...
	*/
	...
	DWORD prId = GetProcessId(piProcessB.hProcess);
	char szPrId[100];
	char szHProc[100];
	sprintf(szPrId, "%d", prId);
	sprintf(szHProc, "%d", piProcessB.hProcess);
	if (!SetEnvironmentVariable("PID_B", szPrId) 
			|| !SetEnvironmentVariable("PHANDLE_B", szHProc)) {
